...
function getSchedule(baseUrl, stop, count, successCb, errorCB) {
    ...
    fetch(baseUrl, {
        method: 'POST',
        body: getHSLPayload(stop.id || stop, count, moment().unix() + (stop.minutesFrom || 0) * 60),
        headers: headers
    })
...
function processStopTimeData(json) {
    ...
    json.stoptimesWithoutPatterns.forEach((value) => {
        ...
        let datVal = new Date(
            (value.serviceDay + value.realtimeDeparture) * 1000
        );
...
const getHeadSign = (headsign) => headsign.includes(" via ") ? headsign.split(" via ").at(0) : headsign;

const getUntil = (date) => Math.floor(moment.duration(date.diff(moment())).asMinutes());
